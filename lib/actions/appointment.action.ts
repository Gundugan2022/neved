...
export const getAppointement = async (appointmentId: string) => {
    try {
...
export const updateAppointment = async ({appointmentId, userId, appointment, type}: UpdateAppointmentParams) => {
  try {
   const updatedAppontment = await databases.updateDocument(
    DATABASE_ID!,
    APPOINTMENT_COLLECTION_ID!,
    appointmentId,
    appointment
   ...
   return parseStringify(updatedAppontment)
  } catch(error) {
    console.log(error)
  }
}
